...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-72 rounded-lg border border-hacman-gray bg-hacman-dark shadow-xl shadow-black/40">
          <div className="border-b border-hacman-gray px-3 py-2 text-sm font-medium text-gray-300">
            Notifications
          </div>
          {items.length === 0 ? (
            <p className="px-3 py-4 text-center text-xs text-hacman-muted">No notifications</p>
          ) : (
              ...
              {items.map((n) => (
                <li key={n.id}>
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-xs hover:bg-hacman-gray transition-colors ${
                      n.isRead ? "text-hacman-muted" : "font-medium text-gray-300"
                    }`}
                  >
                    <span className="block">{n.type}</span>
                    <span className="text-[10px] text-hacman-muted">
